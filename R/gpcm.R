...
# # k0[,1] <- -b0 # constrain
#
# test <- ltm::gpcm(Science[c(1,3,4,7)])
# k0[,2:4] <- test$coefficients %>% purrr::map_df(as.numeric) %>% dplyr::slice(-4) %>% t()
# rm(prob)
#
...
#   X <- tibble::tibble(node = c(1:M) %>% rep(rep(sum(unlist(max_cat_item)), M)),
...
#       X1 <- X %>% dplyr::filter(item == j, category >= k)
#       gr_k[k] <- sum(a0[j] * (rr[j, X1$category, X1$node] - X1$P * r_bar[j, X1$node])) # equation 6.26
#
#       X1 <- X1 %>% group_by(node) %>% dplyr::summarise(cumP = sum(P))
#       for(l in 1:max_cat_item[[j]]){
#         X0 <- X %>% dplyr::filter(item == j, category >= l) %>% group_by(node) %>% dplyr::summarise(cumP = sum(P))
...
# }
#
# M2_gpcm(a0, b0, k0, cat_item, xm, rjkm, Nm)
#
#
# #
